...
mod tests {
    ...
    fn hex_eq(bytes: &[u8], expected: &str) -> bool {
        if expected.len() != bytes.len() * 2 {
            return false;
        }
        const LUT: &[u8; 16] = b"0123456789abcdef";
        expected.as_bytes().chunks(2).zip(bytes).all(|(pair, b)| {
            pair[0] == LUT[(b >> 4) as usize] && pair[1] == LUT[(b & 0xf) as usize]
        })
    }

    const EMPTY_LEVEL_HEX: &str = "";
    const OCCUPIED_LEVEL_HEX: &str = "";
    const BITMAP_HEX: &str = "";

    #[test]
    fn js_fixtures() {
        assert!(hex_eq(&Level::default().encode(), EMPTY_LEVEL_HEX));
        ...
        let level = Level {
            ...
            head_seq: 5,
            tail_seq: 9,
            head_consumed_lots: 7,
            open_lots: 123456789012,
            slots,
        };
        assert!(hex_eq(&level.encode(), OCCUPIED_LEVEL_HEX));
        ...
        bm.set(8);
        bm.set(2047);
        assert!(hex_eq(&bm.encode(), BITMAP_HEX));
    }
}
